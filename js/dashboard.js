document.addEventListener('DOMContentLoaded', () => {
    ...
    let lapDataCache = {};
    let plotDataCache = {};
    let isSyncingLayout = false; // Flag para prevenir recursión en relayout
    ...
    async function displaySelectedData() {
        ...
        try {
                ...
                if (lapDataCache[lapCountOriginalFile]) {
                    ...
                    if (lapCountResponse.ok) {
                        ...
                        if (lapCountText.trim()) {
                            ...
                            }).filter(Boolean).sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
            ...
            if (!carDataText.trim()) { if (loadingPlotsMsgDiv) loadingPlotsMsgDiv.remove(); plotArea.innerHTML = "<p>No hay datos de CarData para procesar.</p>"; return; }
            const lines = carDataText.trim().split('\n');
            const allCarDataEntries = lines.map(line => { try { return JSON.parse(line); } catch (e) { console.warn("Error parseando línea de CarData:", e, line); return null; } }).filter(Boolean);

            if (allCarDataEntries.length === 0) { if (loadingPlotsMsgDiv) loadingPlotsMsgDiv.remove(); plotArea.innerHTML = "<p>No se encontraron entradas válidas en CarData.</p>"; return; }
            ...
            } else if (!driverTla) { /* Mensaje si no hay piloto seleccionado */ }
            ...
            if (pilotCarData.length > 0) {
                ...
                let xAxisType = 'date'; // Plotly usa 'date' para ejes de tiempo
                ...
                if (selectedXAxisType === 'lap' && lapData && lapData.length > 0) {
                    xAxisTitle = 'Vuelta';
                    xAxisType = 'linear'; // Para números de vuelta
                    ...
                    pilotCarData.forEach(carEntry => {
                        ...
                        for (let i = 0; i < lapData.length; i++) {
                            ...
                            if (carEntry.Utc >= lapStartTimestamp) {
                                ...
                                assignedLap = (lapData[0].lap > 0) ? lapData[0].lap - 1 : 0; // Outlap/Vuelta 0
                                break;
                            }
                        }
                        if (assignedLap === undefined && lapData.length > 0) {
                            assignedLap = lapData[lapData.length - 1].lap;
                        }
                ...
                function createOrUpdatePlot(divId, traces, layoutConfig, dataKey, defaultName) {
                    ...
                    const validDataTraces = traces.map(trace => {
                        ...
                        trace.x.forEach((xVal, i) => {
                            if (xVal !== undefined && trace.y[i] !== undefined && trace.y[i] !== null) {
                                validIndices.push(i);
                            }
                        });
                        return {
                            ...trace,
                            x: validIndices.map(i => trace.x[i]),
                            y: validIndices.map(i => trace.y[i])
                        };
                    ...
                    plotDataCache[dataKey] = {
                        xValues: validDataTraces.length > 0 ? validDataTraces[0].x : [], // Asegurar que xValues existe
                ...
                plotConfigs.forEach(p => {
                    ...
                    if (p.id === 'drs' && validValues.length > 0) yAxisConfig.range = [-0.5, Math.max(0, ...validValues) + 0.5]; else if (p.id === 'drs') yAxisConfig.range = [-0.5, 2.5];
                    ...
                    createOrUpdatePlot(p.id,
                        [{ x: xAxisData, y: values, type: 'scattergl', mode: 'lines', name: channelMapping[p.chan].name, line: { color: p.color, shape: p.lineShape || 'linear' }, channelIdForHover: p.chan }],
                ...
                const tbTraces = [];
                if (throttleValues.some(v => v !== undefined && v !== null)) tbTraces.push({ x: xAxisData, y: throttleValues, type: 'scattergl', mode: 'lines', name: channelMapping['4'].name, line: { color: 'blue' }, channelIdForHover: '4' });
                if (brakeValues.some(v => v !== undefined && v !== null)) tbTraces.push({ x: xAxisData, y: brakeValues, type: 'scattergl', mode: 'lines', name: channelMapping['5'].name, line: { color: 'orange' }, channelIdForHover: '5' });
                ...
                activePlotDivs.forEach(masterPlotDiv => {
                    masterPlotDiv.on('plotly_relayout', (eventData) => {
                        if (isSyncingLayout) {
                            // console.log('Relayout event ignored due to active sync:', masterPlotDiv.id);
                            return;
                        }
                        // console.log('Relayout event on:', masterPlotDiv.id, eventData);
                        ...
                        if (eventData['xaxis.range[0]'] !== undefined && eventData['xaxis.range[1]'] !== undefined) {
                            ...
                            newXRange['xaxis.autorange'] = true;
                        }

                        if (Object.keys(newXRange).length > 0) {
                            activePlotDivs.forEach(slavePlotDiv => {
                                if (slavePlotDiv !== masterPlotDiv && slavePlotDiv.data && slavePlotDiv.data.length > 0) {
                                    // console.log('Syncing relayout from', masterPlotDiv.id, 'to', slavePlotDiv.id, newXRange);
                                    Plotly.relayout(slavePlotDiv, newXRange);
                                }
                            });
                        }
                        
                        // Reset flag after a short delay to allow Plotly to process internal events
                        setTimeout(() => {
                            // console.log('Resetting isSyncingLayout flag from event on:', masterPlotDiv.id);
                            isSyncingLayout = false;
                        }, 0); // Using 0ms timeout to push to end of event queue
                    });
                    ...
                    masterPlotDiv.on('plotly_hover', (eventData) => {
                        ...
                        activePlotDivs.forEach(plotDiv => {
                            if (!(plotDiv && plotDiv.layout && plotDiv.data && plotDiv.data.length > 0)) return; // Ensure plot is valid
                            ...
                            if (plotDataCache[plotKey] && plotDataCache[plotKey].xValues && plotDataCache[plotKey].xValues.length > 0) {
                                ...
                                if (dataForPlot.xValues.length > 0) {
                                    ...
                                    dataForPlot.xValues.forEach((val, idx) => {
                                        ...
                                        const diff = currentLayout.xaxis.type === 'date' ?
                                            Math.abs(new Date(val).getTime() - new Date(xHoverValue).getTime()) :
                                            Math.abs(val - xHoverValue);
                                        if (diff < minDiff) {
                                            minDiff = diff;
                                            closestIndex = idx;
                                        } else if (diff === minDiff && currentLayout.xaxis.type === 'linear' && val === xHoverValue) {
                                            closestIndex = idx;
                                        }
                                    });
                                    ...
                                    if (currentLayout.hovermode === 'closest' && eventData.points[0].pointNumber !== undefined) {
                                        ...
                                        if (dataForPlot.xValues[eventPointIndex] !== undefined &&
                                            (currentLayout.xaxis.type === 'date' || Math.abs(dataForPlot.xValues[eventPointIndex] - xHoverValue) < 0.01) ) {
                                ...
                                if (closestIndex !== -1 && dataForPlot.xValues[closestIndex] !== undefined) {
                                    dataForPlot.traces.forEach(trace => {
                                        if (!trace || trace.y === undefined || trace.y === null) return; // Ensure trace and y data exist
                                        ...
                                        if (yValue !== undefined && yValue !== null) {
                                            ...
                                            if (currentLayout.xaxis && currentLayout.xaxis.range && currentLayout.xaxis.range.length === 2) {
                                                ...
                                                const xHoverComparable = currentLayout.xaxis.type === 'date' ? new Date(xHoverValue).getTime() : xHoverValue;
                                                const rangeStartComparable = currentLayout.xaxis.type === 'date' ? new Date(xRange[0]).getTime() : xRange[0];
                                                const rangeEndComparable = currentLayout.xaxis.type === 'date' ? new Date(xRange[1]).getTime() : xRange[1];
                                            ...
                                            newAnnotations.push({
                                                x: dataForPlot.xValues[closestIndex], 
                                                y: yValue, xref: 'x', yref: 'y',
                                                text: annotationText,
                    ...
                    masterPlotDiv.on('plotly_unhover', () => {
                        activePlotDivs.forEach(plotDiv => {
                             if (!(plotDiv && plotDiv.layout)) return; // Ensure plot and layout exist
                            const currentLayout = plotDiv.layout;
                            const shapes = (currentLayout.shapes || []).filter(s => s.name !== 'hover-line');
                            const annotations = (currentLayout.annotations || []).filter(a => a.name !== 'hover-annotation');
    ...
    driverSelect.addEventListener('change', () => {
        ...
        if (plotArea.childElementCount === 0 || (plotArea.firstElementChild && plotArea.firstElementChild.tagName === 'P')) {
    ...
    document.querySelectorAll('input[name="xAxisType"]').forEach(radio => {
        radio.addEventListener('change', () => {
            const plotAreaHasContent = Object.values(plotDivsMap).some(div => div && div.data && div.data.length > 0);
            ...
            else if (yearSelect.value && gpSelect.value && sessionSelect.value && (loadDataButton.disabled || !plotAreaHasContent)) {
